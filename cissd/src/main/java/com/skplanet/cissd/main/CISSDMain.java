package com.skplanet.cissd.main;
...
import com.skplanet.cissd.comm.PipelineMgr;

/**
 * Main class of the CISSD.
 * 
 * @author leejy
 * 
 */
final class CISSDMain
{
    ...
    {
        Logger sLog = LoggerFactory.getLogger(CISSDMain.class);
        sLog.info("Starting");

        final ArgParser sArgParser = new ArgParser();
        addCommonArgs(sArgParser);
        sArgParser.addOption("--port", "NUM", "CISSD TCP port to listen on.");
        ...
        // setDirectoryInSystemProps("cissd.http.webroot",
        // sArgParser.get("--webroot"),
        ...
        try
        {
            ...
            final CISSD sCissd = new CISSD(sClient, sTable, sUidTable);
            regShutdownHook(sCissd);

            final ServerBootstrap sServer = new ServerBootstrap(sFactory);
            sServer.setPipelineFactory(new PipelineMgr(sCissd));
            ...
            final int aRet)
    {
        ...
        System.err.println("# Usage: cissd --port=PORT\n");
        if (aArgp != null)
        {
    ...
    private static void regShutdownHook(final CISSD aCissd)
    {
        final class CISSDShutdown extends Thread
        {
            public CISSDShutdown()
            {
                super("CISSDShutdown");
            }
            ...
            public void run()
            {
                try
                {
                    aCissd.shutdown().join();
                }
                catch (Exception e)
                {
                    LoggerFactory.getLogger(CISSDShutdown.class).error(
                            "Uncaught exception during shutdown", e);
                }
            }
        }
        Runtime.getRuntime().addShutdownHook(new CISSDShutdown());
    }

    /** Adds common CISSD options to the given {@code argp}. */
    ...
    static void addAutoMetricArg(final ArgParser aArgp)
    {
        aArgp.addOption("--auto-metric",
                "Automatically add metrics to cissd as they"
    ...
    static HBaseClient getHBaseClient(final ArgParser aArgp)
    {
        ...
        {
            System.setProperty("cissd.core.auto_create_metrics", "true");
        }
