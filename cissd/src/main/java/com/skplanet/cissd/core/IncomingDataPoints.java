...
import com.skplanet.ciscommon.dataop.DataPointsIterator;
import com.skplanet.ciscommon.dataop.SeekableView;
import com.skplanet.ciscommon.dataop.WritableDataPoints;
...
final class IncomingDataPoints implements WritableDataPoints
{
    ...
    /** For auto create metrics mode, set by --auto-metric flag in CISSDMain. */
    private static final boolean AUTO_METRIC = System
            .getProperty("cissd.core.auto_create_metrics") != null;
    ...
    /** The {@code CISSD} instance we belong to. */
     ...
     * 
     * @param cissd
     *            The CISSD we belong to.
     */
    IncomingDataPoints(final CISSD cissd)
    {
        this.cissd = cissd;
     ...
     */
    static byte[] rowKeyTemplate(final CISSD cissd, final String metric,
            final Map<String, String> tags)
    {
        final short metric_width = cissd.metrics.width();
        final short tag_name_width = cissd.tag_names.width();
        final short tag_value_width = cissd.tag_values.width();
        ...
        copyInRowKey(row, pos,
                (AUTO_METRIC ? cissd.metrics.getOrCreateId(metric)
                        : cissd.metrics.getId(metric)));
        pos += metric_width;

        pos += Const.TIMESTAMP_BYTES;

        for (final byte[] tag : Tags.resolveOrCreateAll(cissd, tags))
        {
    ...
    public void setSeries(final String metric, final Map<String, String> tags)
    {
        ...
        row = rowKeyTemplate(cissd, metric, tags);
        size = 0;
    }
    ...
    private long updateBaseTime(final long timestamp)
    {
        ...
        // so that all CISSDs create rows with the same base time. Otherwise
        // we'd need to coordinate CISSDs to avoid creating rows that cover
        ...
        Bytes.setInt(row, (int) base_time, cissd.metrics.width());
        // LJY
        // cissd.scheduleForCompaction(row, (int) base_time);
        return base_time;
    }
            ...
            final byte[] value, final short flags)
    {
        ...
        final PutRequest point = new PutRequest(cissd.table, row, CISSD.FAMILY,
        ...
        // TODO: Add an errback to handle some error cases here.
        point.setDurable(!batch_import);
        return cissd.client.put(point)/* .addBoth(cb) */;
    }
    ...
    {
        return Bytes.getUnsignedInt(row, cissd.metrics.width());
    }
    ...
    public void setBufferingTime(final short time)
    {
        ...
        cissd.client.setFlushInterval(time);
    }
    ...
    public void setBatchImport(final boolean batchornot)
    {
        ...
        final long current_interval = cissd.client.getFlushInterval();
        if (batchornot)
        {
            batch_import = true;
    ...
    public String metricName()
    {
        ...
        final byte[] id = Arrays.copyOfRange(row, 0, cissd.metrics.width());
        return cissd.metrics.getName(id);
    }

    public Map<String, String> getTags()
    {
        return Tags.getTags(cissd, row);
    }

    /* BUG-39613 : Add Legend clause in CISQL */
    public ArrayList<SimpleEntry<String, String>> getAllTags()
    {
        return Tags.getAllTags(cissd, row);
    }
