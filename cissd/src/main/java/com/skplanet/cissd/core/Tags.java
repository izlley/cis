...
import com.skplanet.ciscommon.uid.NoSuchUniqueId;
...
public final class Tags
{
     ...
     * 
     * @param cissd
     *            The CISSD instance to use for UniqueId lookups.
     * @param row
     ...
     */
    static String getValue(final CISSD cissd, final byte[] row,
            final String name) throws NoSuchUniqueName
    {
        validateString("tag name", name);
        final byte[] id = cissd.tag_names.getId(name);
        final byte[] value_id = getValueId(cissd, row, id);
        if (value_id == null)
        {
            return null;
        }
        ...
        try
        {
            return cissd.tag_values.getName(value_id);
        }
     ...
     */
    static byte[] getValueId(final CISSD cissd, final byte[] row,
            final byte[] tag_id)
    {
        final short name_width = cissd.tag_names.width();
        final short value_width = cissd.tag_values.width();
        // BUGBUG: Can do a binary search.
        for (short pos = (short) (cissd.metrics.width() + Const.TIMESTAMP_BYTES); pos < row.length; pos += name_width
                + value_width)
        {
     ...
     * 
     * @param cissd
     *            The CISSD instance to use for Unique ID lookups.
     * @param row
     ...
     */
    static Map<String, String> getTags(final CISSD cissd, final byte[] row)
            throws NoSuchUniqueId
    {
        final short name_width = cissd.tag_names.width();
        final short value_width = cissd.tag_values.width();
        ...
        final short metric_ts_bytes = (short) (cissd.metrics.width() + Const.TIMESTAMP_BYTES);
        ...
        for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes)
        {
            ...
            final String name = cissd.tag_names.getName(tmp_name);
            System.arraycopy(row, pos + name_width, tmp_value, 0, value_width);
            final String value = cissd.tag_values.getName(tmp_value);
            result.put(name, value);
        }
        return result;
    }

    static ArrayList<SimpleEntry<String, String>> getAllTags(final CISSD cissd, final byte[] row)
            throws NoSuchUniqueId
    {
        final short name_width = cissd.tag_names.width();
        final short value_width = cissd.tag_values.width();
        ...
        final byte[] tmp_value = new byte[value_width];
        final short metric_ts_bytes = (short) (cissd.metrics.width() + Const.TIMESTAMP_BYTES);
        final ArrayList<SimpleEntry<String, String>> result = 
        ...
        for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes)
        {
            ...
            SimpleEntry<String, String> entry =
                    new SimpleEntry<String, String>(name, value);
            result.add(entry);
        }
        return result;
    }
     ...
     * 
     * @param cissd
     *            The CISSD to use for UniqueId lookups.
     * @param tags
     *            The tags to resolve.
     ...
     *             name or tag value.
     */
    static ArrayList<byte[]> resolveAll(final CISSD cissd,
            final Map<String, String> tags) throws NoSuchUniqueName
    {
        return resolveAllInternal(cissd, tags, false);
    }
     ...
     * @return an array of sorted tags (tag id, tag name).
     */
    static ArrayList<byte[]> resolveOrCreateAll(final CISSD cissd,
            final Map<String, String> tags)
    {
        return resolveAllInternal(cissd, tags, true);
    }

    private static ArrayList<byte[]> resolveAllInternal(final CISSD cissd,
            ...
            throws NoSuchUniqueName
    {
        ...
        {
            final byte[] tag_id = (create ? cissd.tag_names.getOrCreateId(entry
                    .getKey()) : cissd.tag_names.getId(entry.getKey()));
            final byte[] value_id = (create ? cissd.tag_values
                    .getOrCreateId(entry.getValue()) : cissd.tag_values
     ...
     *             bytes.
     */
    static HashMap<String, String> resolveIds(final CISSD cissd,
