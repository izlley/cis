...
public final class UniqueId implements UniqueIdInterface
{
    ...
    private static final byte[]                     DESC_QF                   = toBytes("desc_m");
    ...
    private static final short                      MAX_SUGGESTIONS           = 9999;

    /** HBase client to use. */
    private final HBaseClient                       client;
    /** Table where IDs are stored. */
    private final byte[]                            table;
    /** Table where DATAs are stored. */
    private final byte[]                            mDatatable;
    ...
    private UniqueId                              mTagkvTable = null;
    
    public static enum mSuggestType { METRICS, TAGV, TAGK };
    ...
    private static final short TIMESTAMP_BYTES = 4;
     ...
     *             empty string.
     */
    public UniqueId(final HBaseClient client, final byte[] table, final byte[] aDatatable,
            final String kind, final int width)
    {
        this.client = client;
        this.table = table;
        this.mDatatable = aDatatable;
        if (kind.isEmpty())
        {
    ...
    public void setTagkvTable( UniqueId aTagTable )
    {
        this.mTagkvTable = aTagTable;
    }
    ...
    public byte[] getOrCreateId(String name) throws HBaseException
    {
        ...
        while (attempt-- > 0)
        {
            ...
            // We now have hbase.regionserver.lease.period ms to complete the
            // loop.

            try
            {
                // Verify that the row still doesn't exist (to avoid re-creating
                // it if
                // it got created before we acquired the lock due to a race
                // condition).
                try
                {
                ...
                try
                {
                    // We want to send an ICV with our explicit RowLock, but
                    // HBase's RPC
                    // interface doesn't expose this interface. Since an ICV
                    // would
                    // attempt to lock the row again, and we already locked it,
                    // we can't
                    // use ICV here, we have to do it manually while we hold the
                    // RowLock.
                    // To be fixed by HBASE-2292.
                    { // HACK HACK HACK
                        {
                            final byte[] current_maxid = hbaseGet(MAXID_ROW,
                                    ID_FAMILY, lock);
                        ...
                        final PutRequest update_maxid = new PutRequest(table,
                                MAXID_ROW, ID_FAMILY, kind, row, lock);
                        hbasePutWithRetry(update_maxid, MAX_ATTEMPTS_PUT,
                                INITIAL_EXP_BACKOFF_DELAY);
                    } // end HACK HACK HACK.
                    LOG.info("Got ID=" + id + " for kind='" + kind()
                            + "' name='" + name + "'");
                    ...
                    for (int i = 0; i < row.length - idWidth; i++)
                    {
                        if (row[i] != 0)
                        {
                            final String message = "All Unique IDs for "
                                    + kind() + " on " + idWidth
                    ...
                    row = Arrays.copyOfRange(row, row.length - idWidth,
                            row.length);
                } catch (HBaseException e)
                {
                    LOG.error(
                ...
                // If we die before the next PutRequest succeeds, we just waste
                // an ID.

                // Create the reverse mapping first, so that if we die before
                // creating
                // the forward mapping we don't run the risk of "publishing" a
                // partially assigned ID. The reverse mapping on its own is
                // harmless
                // but the forward mapping without reverse mapping is bad.
                try
                {
                    final PutRequest reverse_mapping = new PutRequest(table,
                            row, NAME_FAMILY, kind, toBytes(name));
                    hbasePutWithRetry(reverse_mapping, MAX_ATTEMPTS_PUT,
                            INITIAL_EXP_BACKOFF_DELAY);
                } catch (HBaseException e)
                {
                    LOG.error("Failed to Put reverse mapping!  ID leaked: "
                            + id, e);
                    hbe = e;
                    continue;
                }
                ...
                {
                    LOG.error("Failed to Put forward mapping!  ID leaked: "
                            + id, e);
                    hbe = e;
                    continue;
                }
     ...
     */
    public List<String> suggest(final String search, mSuggestType aType) throws HBaseException
    {
        final LinkedList<String> suggestions = new LinkedList<String>();
        
        // TODO: Add caching to try to avoid re-scanning the same thing.
        try
        {
            if( search.length() > 0 )
            {
                if( search.charAt(0) == '*' )
                {
                    switch (aType)
                    {
                        case TAGK:
                            ...
                            {
                                /* q=*metric_name */
                                // return all tag keys correspond to a
                                // metric
                                final Scanner scanner = getTagkSuggestScanner(search
                                        .substring(1));

                                final byte[] sKey = scanner.nextRows(1)
                                        .joinUninterruptibly().get(0).get(0)
                                        .key();
                                for (final String sTag : getTags(sKey))
                                {
                                    suggestions.add(sTag);
                                }
                            }
                            else
                            {
                ...
                {
                    suggestDefault( search, suggestions );
                }
            }
            else
            {
    ...
    public ArrayList<SimpleEntry<String, String>> suggestwithDesc(final String search, mSuggestType aType) throws HBaseException
    {
        final ArrayList<SimpleEntry<String, String>> suggestions = new ArrayList<SimpleEntry<String, String>>();
        ...
        try
        {
            ...
            switch (aType)
            {
                case TAGK:
                    ...
                    {
                        // return all tag keys
                        suggestwithDescDefault("", suggestions);
                    }
                    break;
                case TAGV:
                    if (search.length() > 1)
    ...
    private ArrayList<String> getTags( final byte[] row )
            throws NoSuchUniqueId
    {
        final short tag_bytes = (short) (TAG_NAME_WIDTH + TAG_VALUE_WIDTH);
        final byte[] tmp_name = new byte[TAG_NAME_WIDTH];
        final short metric_ts_bytes = (short) (METRICS_WIDTH + TIMESTAMP_BYTES);
        final ArrayList<String> result = 
                new ArrayList<String>();
        for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes)
        {
            System.arraycopy(row, pos, tmp_name, 0, TAG_NAME_WIDTH);
            final String name = this.getName(tmp_name);
            result.add(name); // dummy value
        }
        return result;
    }
    
    private void suggestDefault( String search, LinkedList<String> aSugList )
            throws Exception
    {
        final Scanner scanner = getSuggestScanner(search, false);
        ...
        while ((rows = scanner.nextRows().joinUninterruptibly()) != null)
        {
            for (final ArrayList<KeyValue> row : rows)
            {
                ...
                aSugList.add(name);
                if ((short) aSugList.size() > MAX_SUGGESTIONS)
                {
                    break;
                }
            }
    ...
    private void suggestwithDescDefault( String search, ArrayList<SimpleEntry<String, String>> aSugList )
            throws Exception
    {
        final Scanner scanner = getSuggestScanner(search, true);
            ...
            for (final ArrayList<KeyValue> row : rows)
            {
                if ( (row.size() != 1) && (row.size() != 2) )
                {
                    LOG.error("WTF shouldn't happen!  Scanner "
                            + scanner
                            + " returned"
                            + " a row that doesn't have exactly 1  or 2 KeyValue: "
                            + row);
                    if (row.isEmpty())
                    {
                        continue;
                ...
                if( row.size() == 2 )
                {
                    sDesc = fromBytes( row.get(1).value() );
                }
                final byte[] cached_id = nameCache.get(name);
                if (cached_id == null)
                {
                ...
                aSugList.add( new SimpleEntry<String, String>(name, sDesc) );
                if ((short) aSugList.size() > MAX_SUGGESTIONS)
                {
                    break;
                }
     ...
     */
    private Scanner getSuggestScanner(final String search, final boolean aIsDesc)
    {
        final byte[] start_row;
        final byte[] end_row;
        byte[]   sDesc = this.DESC_QF;
        byte[][] sQfname = new byte[2][];
        if (search.isEmpty())
        {
            start_row = START_ROW;
            end_row = END_ROW;
        ...
        else
        {
            sQfname[0] = kind;
            sQfname[1] = sDesc;
            scanner.setQualifiers( sQfname );
        }
        scanner.setMaxNumRows(MAX_SUGGESTIONS);
        return scanner;
    }
    
    private Scanner getTagkSuggestScanner(final String search)
    {   
    	final byte[] start_row = new byte[METRICS_WIDTH + TIMESTAMP_BYTES];
    	final byte[] end_row = new byte[METRICS_WIDTH + TIMESTAMP_BYTES];
    	Bytes.setInt(start_row, (int)(System.currentTimeMillis()/1000 - 302400), METRICS_WIDTH); // 1week ago
    	Bytes.setInt(end_row, 0xffffffff, METRICS_WIDTH);
    	System.arraycopy(mTagkvTable.getId(search), 0, start_row, 0, METRICS_WIDTH);
    	System.arraycopy(mTagkvTable.getId(search), 0, end_row, 0, METRICS_WIDTH);

        final Scanner scanner = client.newScanner(this.mDatatable);
        scanner.setStartKey(start_row);
        scanner.setStopKey(end_row);
    ...
    /**
     * Gets an exclusive lock for on the table using the MAXID_ROW. The lock
     * expires after hbase.regionserver.lease.period ms (default = 60000)
     * 
     * @throws HBaseException
     *             if the row lock couldn't be acquired.
     */
    private RowLock getLock() throws HBaseException
    {
        try
        {
            return client.lockRow(new RowLockRequest(table, MAXID_ROW))
                    .joinUninterruptibly();
        } catch (HBaseException e)
        {
