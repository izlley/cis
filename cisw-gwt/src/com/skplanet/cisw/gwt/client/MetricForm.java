...
final class MetricForm extends HorizontalPanel implements Focusable
{
    ...
    private static final String CIS_ID_CLASS = "[-_./a-zA-Z0-9]";
    private static final String CIS_ID_RE = "^" + CIS_ID_CLASS + "*$";
    private static final String CIS_TAGVALUE_RE = "^(\\*?" // a `*' wildcard or
                                                           // nothing
            + "|" + CIS_ID_CLASS + "+(\\|" + CIS_ID_CLASS + "+)*)$"; // `foo|bar|...'
    private static final String HTTP_PLUS_CHAR = "%2B";
    ...
    public MetricForm(final EventsHandler handler)
    {
        ...
        metric.setValidationRegexp(CIS_ID_RE);
        assembleUi();
    }
    ...
    public boolean buildQueryString(final StringBuilder url)
    {
        ...
        if( x1y2.getValue() )
        {
            url.append(" ON AXISY2 ");
        }
        
        url.append(" AGGREGATOR ").append(selectedValue(aggregators));
        if (rate.getValue())
        {
            url.append(" OF RATE");
        }
        ...
        {
            url.append(" DOWNSAMPLING ").append(interval.getValue()).append('-')
    ...
    private void addTag(final String default_tagname)
    {
        ...
        final SuggestBox suggesttagk = RemoteOracle.newSuggestBox("tagk",
                tagname);
        final ValidatedTextBox tagvalue = new ValidatedTextBox();
        final SuggestBox suggesttagv = RemoteOracle.newSuggestBox("tagv",
                tagvalue);
        tagname.setValidationRegexp(CIS_ID_RE);
        tagvalue.setValidationRegexp(CIS_TAGVALUE_RE);
    ...
    public void autoSuggestTag(final String tag)
    {
        ...
        for (int row = 1; row < nrows; row++)
        {
            ...
            final SuggestBox tagvalue = ((SuggestBox) tagtable
                    .getWidget(row, 2));
    ...
    private final BlurHandler recompact_tagtable = new BlurHandler()
    {
        public void onBlur(final BlurEvent event)
        {
            ...
            // Try to remove empty lines from the tag table (but never remove
            // the
    ...
    private static String selectedValue(final ListBox list)
    { // They should add
