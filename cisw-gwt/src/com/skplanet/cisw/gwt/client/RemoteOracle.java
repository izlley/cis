...
final class RemoteOracle extends SuggestOracle
{
    ...
    /**
     * Maps an oracle type to its suggestion cache. The cache is in fact a
     * {@link MultiWordSuggestOracle}, which we re-use as its implementation is
     * good (it uses a trie, handles HTML formatting etc.).
     */
    private static final HashMap<String, MultiWordSuggestOracle> caches = new HashMap<String, MultiWordSuggestOracle>();

    /** Maps an oracle type to the queries recently seen for this type. */
    private static final HashMap<String, QueriesSeen> all_queries_seen = new HashMap<String, QueriesSeen>();
    ...
    /**
     * Pending request that arrived while we were still processing `current'. If
     * requests keep coming in while we're processing `current', the last
     ...
     * @param textbox
     *            The text box to wrap to provide suggestions to.
     */
            ...
            final Callback callback)
    {
        ...
        current = callback;
        {
            ...
            // Check if we can serve this from our local cache, without even
            // talking
            // to the server. This is possible if either of those is true:
            // 1. We've already seen this query recently.
            // 2. This new query precedes another one and the user basically
            // just
            ...
            if ((last_query != null && last_query.compareTo(this_query) <= 0 && this_query
                    .compareTo(last_suggestion) < 0)
        ...
        try
        {
            builder.sendRequest(null, new RequestCallback()
            {
                public void onError(final com.google.gwt.http.client.Request r,
                        final Throwable e)
                {
                    current = null; // Something bad happened, drop the current
                                    // request.
                    if (pending_req != null)
                    { // But if we have another waiting...
                        requestSuggestions(pending_req, pending_cb); // ... try
                                                                     // it now.
                    }
                }

                // Need to use fully-qualified names as this class inherits
                // already
                        ...
                        final com.google.gwt.http.client.Response response)
                {
                    if (response.getStatusCode() == com.google.gwt.http.client.Response.SC_OK
                            // Is this response still relevant to what the
                            // requester wants?
                            && requester.getText().startsWith(last_query))
                    {
                        final JSONValue json = JSONParser.parse(response
                                .getText());
                        // In case this request returned nothing, we pretend the
                        // last
                        // suggestion ended with the largest character possible,
                        // so we
                        // won't send more requests to the server if the user
                        // keeps
                        ...
                        if (json != null && json.isArray() != null)
                        {
                            ...
                            final int n = Math.min(request.getLimit(),
                                    results.size());
                            for (int i = 0; i < n; i++)
                            {
                                final JSONValue suggestion = results.get(i);
                                if (suggestion == null
                                        || suggestion.isString() == null)
                                {
                                    continue;
                                }
                                final String suggestionstr = suggestion
                    ...
                    current = null; // Regardless of what happened above, this
                                    // is done.
                    if (pending_req != null)
                    {
    ...
    private static final class QueriesSeen
    {
        ...
        /**
         * A circular buffer containing the last 32 requests already served. It
         * would be awesome if {@code gwt.user.client.ui.PrefixTree} wasn't
        ...
        {
            // Check most recent queries first, as they're the most likely to
            // match
            // if the user goes back and forth by typing a few characters,
            // removing
