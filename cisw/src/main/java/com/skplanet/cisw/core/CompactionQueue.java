...
import com.skplanet.ciscommon.dataop.IllegalDataException;
...
final class CompactionQueue extends ConcurrentSkipListMap<byte[], Boolean>
{
    ...
    /** The {@code CISW} instance we belong to. */
     ...
     * 
     * @param cisw
     *            The CISW we belong to.
     */
    public CompactionQueue(final CISW cisw)
    {
        super(new Cmp(cisw));
        this.cisw = cisw;
        metric_width = cisw.metrics.width();
        if (CISW.enable_compactions)
        {
    ...
    private Deferred<ArrayList<Object>> flush(final long cut_off, int maxflushes)
    {
        ...
        for (final byte[] row : this.keySet())
        {
            ...
            ds.add(cisw.get(row).addCallbacks(compactcb, handle_read_error));
        }
        ...
        if (nflushes == MAX_CONCURRENT_FLUSHES && maxflushes > 0)
        {
            ...
            // to kick off more.
            cisw.flush(); // Speed up this batch by telling the client to flush.
            ...
            final KeyValue[] compacted)
    {
        ...
        {
            if (row.isEmpty())
            { // Maybe the row got deleted in the mean time?
        ...
        if (compacted != null)
        { // Caller is interested in the compacted form.
            compacted[0] = compact;
        ...
        if (write)
        {
            ...
            return cisw.put(key, qual, value).addCallbacks(
     ...
     * <p>
     * CISW used to not rely on the size recorded in the flags being correct,
     * and so for a long time it was setting the wrong size for floating point
     * values (pretending they were encoded on 8 bytes when in fact they were on
     * 4). So overwrite these bits here to make sure they're correct now,
     * because once they're compacted it's going to be quite hard to tell if the
     * flags are right or wrong, and we need them to be correct to easily decode
     * the values.
     * 
     * @param flags
     ...
     * <p>
     * CISW used to encode all floating point values as `float' (4 bytes) but
     * actually store them on 8 bytes, with 4 leading 0 bytes, and flags
     ...
     * for various reasons, such as CISW dying in the middle of a compaction or
     * races involved with CISW trying to compact the same row at the same time,
     * or old data being slowly written to a CISW.
     * 
     * @param row
     *            The row to compact. Assumed to have 2 elements or more.
            ...
            final ArrayList<KeyValue> row, final int estimated_nvalues)
    {
        ...
        for (final KeyValue kv : row)
        {
            ...
            // this compacted cell has been written by a future version of CISW
            ...
            if (val[val.length - 1] != 0)
            {
                throw new IllegalDataException(
                        "Don't know how to read this value:"
                                ...
                                + " version of CISW, or could be corrupt.");
            }
    ...
    private final class DeleteCompactedCB implements Callback<Object, Object>
    {
        ...
        {
            return cisw.delete(key, qualifiers).addErrback(handle_delete_error);
        }
    ...
    private final class HandleErrorCB implements Callback<Object, Exception>
    {
        ...
        public Object call(final Exception e)
        {
            ...
            { // HBase isn't keeping up.
                ...
                { // Should never get in this clause.
            ...
            if (++errors % 100 == 1)
            { // Basic rate-limiting to not flood logs.
        ...
        public void run()
        {
            ...
            while (true)
            {
                try
                {
                    final long now = System.currentTimeMillis();
                    final int size = size();
                    // Let's suppose MAX_TIMESPAN = 1h. We have `size' rows to
                    // compact,
                    // and we better compact them all before in less than 1h,
                    // otherwise
                    // we're going to "fall behind" when a new hour start (as
                    // we'll be
                    // creating a ton of new rows then). So slice MAX_TIMESPAN
                    // using
                    // FLUSH_INTERVAL to compute what fraction of `size' we need
                    // to
                    // flush at each iteration. Note that `size' will usually
                    // account
                    // for many rows that can't be flushed yet (not old enough)
                    // so we're
                    // overshooting a bit (flushing more aggressively than
                    // necessary).
                    // This isn't a problem at all. The only thing that matters
                    // is that
                    // the rate at which we flush stuff is proportional to how
                    // much work
                    // is sitting in the queue. The multiplicative factor
                    // FLUSH_SPEED
                    // is added to make flush even faster than we need. For
                    // example, if
                    // FLUSH_SPEED is 2, then instead of taking 1h to flush what
                    // we have
                    // for the previous hour, we'll take only 30m. This is
                    // desirable so
                    // that we evict old entries from the queue a bit faster.
                    final int maxflushes = Math
                            .max(MIN_FLUSH_THRESHOLD, size * FLUSH_INTERVAL
                                    * FLUSH_SPEED / Const.MAX_TIMESPAN);
                    // Flush if either (1) it's been too long since the last
                    // flush
                    // or (2) we have too many rows to recompact already.
                    // Note that in the case (2) we might not be able to flush
                    // anything
