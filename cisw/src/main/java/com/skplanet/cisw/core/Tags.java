...
public final class Tags
{
     ...
     * 
     * @param cisw
     *            The CISW instance to use for UniqueId lookups.
     * @param row
     ...
     */
    static String getValue(final CISW cisw, final byte[] row, final String name)
            throws NoSuchUniqueName
    {
        validateString("tag name", name);
        final byte[] id = cisw.tag_names.getId(name);
        final byte[] value_id = getValueId(cisw, row, id);
        if (value_id == null)
        {
            return null;
        }
        ...
        try
        {
            return cisw.tag_values.getName(value_id);
        }
     ...
     */
    static byte[] getValueId(final CISW cisw, final byte[] row,
            final byte[] tag_id)
    {
        final short name_width = cisw.tag_names.width();
        final short value_width = cisw.tag_values.width();
        // TODO: Can do a binary search.
        for (short pos = (short) (cisw.metrics.width() + Const.TIMESTAMP_BYTES); pos < row.length; pos += name_width
                + value_width)
        {
     ...
     * 
     * @param cisw
     *            The CISW instance to use for Unique ID lookups.
     * @param row
     ...
     */
    static Map<String, String> getTags(final CISW cisw, final byte[] row)
            throws NoSuchUniqueId
    {
        final short name_width = cisw.tag_names.width();
        final short value_width = cisw.tag_values.width();
        ...
        final short metric_ts_bytes = (short) (cisw.metrics.width() + Const.TIMESTAMP_BYTES);
        ...
        for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes)
        {
            ...
            final String name = cisw.tag_names.getName(tmp_name);
            System.arraycopy(row, pos + name_width, tmp_value, 0, value_width);
            final String value = cisw.tag_values.getName(tmp_value);
            result.put(name, value);
        }
        return result;
    }

    static ArrayList<SimpleEntry<String, String>> getAllTags(final CISW cisw, final byte[] row)
            throws NoSuchUniqueId
    {
        final short name_width = cisw.tag_names.width();
        final short value_width = cisw.tag_values.width();
     ...
     * 
     * @param cisw
     *            The CISW to use for UniqueId lookups.
     * @param tags
     *            The tags to resolve.
     ...
     *             name or tag value.
     */
    static ArrayList<byte[]> resolveAll(final CISW cisw,
            final Map<String, String> tags) throws NoSuchUniqueName
    {
        return resolveAllInternal(cisw, tags, false);
    }
     ...
     * @return an array of sorted tags (tag id, tag name).
     */
    static ArrayList<byte[]> resolveOrCreateAll(final CISW cisw,
            final Map<String, String> tags)
    {
        return resolveAllInternal(cisw, tags, true);
    }

    private static ArrayList<byte[]> resolveAllInternal(final CISW cisw,
            ...
            throws NoSuchUniqueName
    {
        ...
        {
            final byte[] tag_id = (create ? cisw.tag_names.getOrCreateId(entry
                    .getKey()) : cisw.tag_names.getId(entry.getKey()));
            final byte[] value_id = (create ? cisw.tag_values
                    .getOrCreateId(entry.getValue()) : cisw.tag_values
     ...
     *             bytes.
     */
    static HashMap<String, String> resolveIds(final CISW cisw,
