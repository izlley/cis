 ...
 */
final class CiswQuery implements Query
{

    private static final Logger LOG = LoggerFactory.getLogger(CiswQuery.class);
    ...
    /** Constructor. */
    public CiswQuery(final CISW cisw)
    {
        this.cisw = cisw;
    }
    ...
    {
        findGroupBys(tags);
        this.metric = cisw.metrics.getId(metric);
        this.tags = Tags.resolveAll(cisw, tags);
        aggregator = function;
        this.rate = rate;
    }
    ...
    private void findGroupBys(final Map<String, String> tags)
    {
        ...
        while (i.hasNext())
        {
            ...
            { // Multiple possible values.
                ...
                group_bys.add(cisw.tag_names.getId(tag.getKey()));
                i.remove();
                ...
                group_by_values.put(cisw.tag_names.getId(tag.getKey()),
                        value_ids);
                for (int j = 0; j < values.length; j++)
                {
                    final byte[] value_id = cisw.tag_values.getId(values[j]);
    ...
    {
        final short metric_width = cisw.metrics.width();
        ...
        try
        {
            ...
            while ((rows = scanner.nextRows().joinUninterruptibly()) != null)
            {
                ...
                for (final ArrayList<KeyValue> row : rows)
                {
                    ...
                    if (datapoints == null)
                    {
                        datapoints = new Span(cisw);
                        spans.put(key, datapoints);
                    }
                    datapoints.addRow(cisw.compact(row));
                    nrows++;
    ...
    private DataPoints[] groupByAndAggregate(final TreeMap<byte[], Span> spans)
    {
        ...
        if (group_bys == null)
        {
            ...
            final SpanGroup group = new SpanGroup(cisw, getScanStartTime(),
        ...
        for (final Map.Entry<byte[], Span> entry : spans.entrySet())
        {
            ...
            byte[] value_id = null;
            int i = 0;
            // TODO: The following loop has a quadratic behavior. We can
            // make it much better since both the row key and group_bys are
            // sorted.
            for (final byte[] tag_id : group_bys)
            {
                value_id = Tags.getValueId(cisw, row, tag_id);
                if (value_id == null)
                {
                    break;
                }
            ...
            if (thegroup == null)
            {
                thegroup = new SpanGroup(cisw, getScanStartTime(),
    ...
    Scanner getScanner() throws HBaseException
    {
        ...
        final Scanner scanner = cisw.client.newScanner(cisw.table);
    ...
    void createAndSetFilter(final Scanner scanner)
    {
        ...
        buf.append("(?s)" // Ensure we use the DOTALL flag.
                ...
                .append(cisw.metrics.width() + Const.TIMESTAMP_BYTES)
                .append("}");
    ...
    public String toString()
    {
        ...
        buf.append("CiswQuery(start_time=").append(getStartTime())
                .append(", end_time=").append(getEndTime()).append(", metric=")
                .append(Arrays.toString(metric));
        try
        {
            buf.append(" (").append(cisw.metrics.getName(metric));
        }
        ...
        try
        {
            buf.append("), tags=").append(Tags.resolveIds(cisw, tags));
        }
        catch (NoSuchUniqueId e)
        {
        ...
        if (group_bys != null)
        {
            for (final byte[] tag_id : group_bys)
            {
                try
                {
                    buf.append(cisw.tag_names.getName(tag_id));
                }
                catch (NoSuchUniqueId e)
                {
                    buf.append('<').append(e.getMessage()).append('>');
                ...
                if (group_by_values != null)
                {
                    ...
                    for (final byte[] value_id : value_ids)
                    {
                        try
                        {
                            buf.append(cisw.tag_values.getName(value_id));
                        }
                        catch (NoSuchUniqueId e)
                        {
                            buf.append('<').append(e.getMessage()).append('>');
    ...
    private static final class SpanCmp implements Comparator<byte[]>
    {
        ...
        public int compare(final byte[] a, final byte[] b)
        {
            ...
            if (a == b)
            { // Do this after accessing a.length and b.length
